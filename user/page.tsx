...
export default function UserDashboard() {
  ...
  const [quickStationId, setQuickStationId] = useState<string>("");
  const [quickCargoCount, setQuickCargoCount] = useState<string>("");
  const [quickTotalWeight, setQuickTotalWeight] = useState<string>("");
  const [quickTargetDate, setQuickTargetDate] = useState<string>("");
  ...
  useEffect(() => {
    ...
    setQuickTargetDate(tomorrow.toISOString().slice(0, 10));
  }, []);
  ...
  async function loadMyRequests() {
    ...
    try {
      ...
      const data = await apiGet<CargoRequestItem[]>("/requests/me", undefined, {
        "X-User-Email": email,
      });
  ...
  async function handleQuickSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!quickStationId) {
      setAlert("Lütfen istasyon seçin.");
      return;
    }
    ...
    try {
      ...
      await apiPost(
        "/requests",
        {
          station_id: Number(quickStationId),
          cargo_count: Number(quickCargoCount),
          total_weight_kg: Number(quickTotalWeight),
          target_date: quickTargetDate,
        },
        undefined,
        { "X-User-Email": email }
      );

      setAlert("Talep oluşturuldu.");
      setQuickStationId("");
      setQuickCargoCount("");
      setQuickTotalWeight("");
      const tomorrow = new Date();
  ...
  const stats = useMemo(() => {
    ...
    const tomorrowRequests = requests.filter((r) => {
      const d = new Date(r.target_date);
      return isSameDay(d, tomorrow);
    });

    const tomorrowTotalKg = Math.round(
      tomorrowRequests.reduce((sum, r) => sum + (r.total_weight_kg || 0), 0)
    );
    const tomorrowTotalPiece = tomorrowRequests.reduce(
    ...
    return [
      ...
      {
        label: "Planlanan Talepler",
        value: String(plannedCount),
        color: "from-emerald-500 to-green-500",
        status: "PLANNED",
      },
      {
        label: "Yarınki Toplam Yük",
        value: `${tomorrowTotalKg} kg`,
        color: "from-cyan-500 to-blue-500",
      },
      {
        label: "Yarınki Toplam Adet",
        value: String(tomorrowTotalPiece),
              ...
              <form onSubmit={handleQuickSubmit} className="space-y-4">
                <div>
                  ...
                  {loadingStations ? (
                    ...
                    <select
                      ...
                      required
                      className="w-full rounded-xl border border-white/20 bg-slate-900/40 backdrop-blur-sm px-4 py-3 text-white appearance-none outline-none focus:ring-2 focus:ring-cyan-400/60"
                    >
                ...
                <div>
                  ...
                  <input
                    ...
                    required
                    min="1"
                    value={quickCargoCount}
                    onChange={(e) => setQuickCargoCount(e.target.value)}
                ...
                <div>
                  ...
                  <input
                    ...
                    min="1"
                    step="0.1"
                    value={quickTotalWeight}
                    onChange={(e) => setQuickTotalWeight(e.target.value)}
                    className="w-full rounded-xl border border-white/20 bg-white/10 px-4 py-3 text-white placeholder:text-white/40 outline-none focus:ring-2 focus:ring-cyan-400/60"
                  />
                </div>
                <div>
                ...
                <button
                  type="submit"
                  disabled={
                    quickSubmitting || loadingStations || stations.length === 0
                  }
                ...
                >
                  {quickSubmitting ? "Oluşturuluyor..." : "Talep Oluştur"}
                </button>
